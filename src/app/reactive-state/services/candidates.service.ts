...
import { BehaviorSubject, catchError, delay, map, Observable, of, switchMap, take, tap } from "rxjs";
...
export class CandidatesService {
    ...
    hireCandidate(id: number): void {
        this.candidates$.pipe(
            ...
            delay(1000),
            switchMap(updatedCandates => this.http.patch(`${environment.apiUrl}/candidates/${id}`, updatedCandates.find(candidate => candidate.id === id)),)
        ).subscribe();
    }
}